...
static void zb_send_command(zb_uint8_t param, zb_uint8_t command, int with_params) 
{
  ...
  if (with_params)
  { 
    parameters_cp command_data;
    command_data.command_code = command;
    command_data.parameter = info->choosen_parameter;

    zb_init_buffer(param, info->device_address, &command_data, sizeof(parameters_cp));
  }
  else
  {
...
{
   zb_send_command(param, TURN_ON, 0);
}

void zb_send_turn_off(zb_uint8_t param) ZB_CALLBACK
{
   zb_send_command(param, TURN_OFF, 0);
}
...
{
   zb_send_command(param, CHANGE_BRIGHTNESS_FOR_STEP_DOWN, 0);
}

void zb_send_set_brightness(zb_uint8_t param) ZB_CALLBACK
{
   zb_send_command(param, CHANGE_BRIGHTNESS_TO_VALUE, 1);
}

void zb_send_switch_color(zb_uint8_t param) ZB_CALLBACK
{
   zb_send_command(param, SWITCH_COLOR, 1);
}
